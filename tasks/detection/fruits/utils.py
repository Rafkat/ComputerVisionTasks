...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, locs_pred, cls_pred, boxes, labels):
        ...
        t_locs = torch.zeros((batch_size, n_default_boxes, 4), dtype=torch.float).to(device)
        t_classes = torch.zeros((batch_size, n_default_boxes), dtype=torch.long).to(device)
        ...
        for i in range(batch_size):
            ...
            t_locs[i] = encode_bboxes(xy_to_cxcy(boxes[i][object_each_default_box]), self.default_boxes)

        pos_default_boxes = t_classes != 0
        ...
        confidence_loss_all = cross_entropy_loss(cls_pred.view(-1, num_classes), t_classes.view(-1))
...
label_map = {'background': 0, 'pineapple': 1, 'snake fruit': 2, 'dragon fruit': 3, 'banana': 4}
...
def calculate_mAP(det_boxes, det_labels, det_scores, true_boxes, true_labels):
    ...
    for c in range(1, n_classes):
        ...
        cumul_recall = cumul_true_positives / true_class_images.sum().item()  # (n_class_detections)
